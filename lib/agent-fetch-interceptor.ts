 ...
 */
// Auth-token key written by lyzr-architect-pg's AuthProvider (localStorage).
const AUTH_TOKEN_KEY = 'auth_token'

// True for same-origin `/api/...` requests (the app's own API routes).
function isSameOriginApiPath(u: string): boolean {
...
// Attach `Authorization: Bearer <token>` to the request's init. The token is
// stored by the package AuthProvider in localStorage. This is what makes auth
// work inside the cross-origin preview iframe, where the auth cookie is a
// blocked third-party cookie — Bearer headers are not subject to cookie policy.
...
async function interceptedFetch(
  ...
  // Auto-attach the app's Bearer token to same-origin /api requests so
  // authenticated routes work in the preview iframe (the auth cookie is blocked
  // there as a third-party cookie). Scoped to same-origin /api ONLY — the token
  // is never sent cross-origin, so it can't leak to the Lyzr API or elsewhere.
  // Only for string/URL inputs (leave Request objects, which carry their own
  // headers, untouched).
  if ((typeof input === 'string' || input instanceof URL) && isSameOriginApiPath(url)) {
    init = withBearerToken(init)
  }
