  ...
  const callAgent = useCallback(async (callOptions: AgentCallOptions): Promise<UseAgentResult> => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        // Agent-level failure (200 + success:false) — handled in-app via the
        // error modal; the parent is only notified if the user clicks fix
        // (requestFix). Auto-escalating here would flag the app as broken for
        // upstream inference failures. Genuine server/upstream failures reach
        // the parent through fetchWrapper's 5xx handling instead.
        if (isInIframe() && globalErrorCallback && options.showErrorModal !== false) {
      ...
      // Check for agent-level error (status: 'error') — same in-app-only
      // handling as above.
...
export const callAgentAPI = async (
  ...
  try {
    ...
    if (!result.success || result.response.status === 'error') {
      ...
      // Agent-level failure — in-app modal only; see useAgent above.
      if (isInIframe() && globalErrorCallback) {
        globalErrorCallback(errorDetails)
      }
